...
/**
 * CDI producer that creates the correct {@link ChatLanguageModel} based on configuration.
 *
 * Supported providers:
 * <ul>
 *   <li>{@code openai}  — OpenAI API (GPT-4o, etc.)</li>
 *   <li>{@code claude}  — Anthropic Claude API</li>
 *   <li>{@code vllm}    — Local vLLM with OpenAI-compatible API (uses OpenAI client with custom base URL)</li>
 * </ul>
 *
 * The provider is selected via {@code app.ai.provider} in application.properties.
